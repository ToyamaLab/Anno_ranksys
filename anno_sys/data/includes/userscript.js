    ...
    function : {
        ...
        setEventListener : function(){
            $('.wix-attach').on('click', function(e) {
              if(e.target.getAttribute('wid')=='1'){
                chrome.storage.sync.get("id0", function(data){
                  var saeki_wid = String(Number(data.id0)+120);
                  redirector.setWtId('')
                  //WixU.main.function.attach(e.target.getAttribute('wid'), WixU.option.get('Main', 'minLength'));
                  WixU.main.function.attach(saeki_wid, WixU.option.get('Main', 'minLength'));
                });
              }else if(e.target.getAttribute('wid')=='2'){
                chrome.storage.sync.get("id1", function(data){
                  var saeki_wid = String(Number(data.id1)+120);
                  redirector.setWtId('')
                  WixU.main.function.attach(saeki_wid, WixU.option.get('Main', 'minLength'));
                });
              }else if(e.target.getAttribute('wid')=='3'){
                chrome.storage.sync.get("id2", function(data){
                  var saeki_wid = String(Number(data.id2)+120);
                  redirector.setWtId('')
                  WixU.main.function.attach(saeki_wid, WixU.option.get('Main', 'minLength'));
                });
              }else if(e.target.getAttribute('wid')=='4'){
                chrome.storage.sync.get("id3", function(data){
                  var saeki_wid = String(Number(data.id3)+120);
                  redirector.setWtId('')
                  WixU.main.function.attach(saeki_wid, WixU.option.get('Main', 'minLength'));
                });
              }else if(e.target.getAttribute('wid')=='5'){
                chrome.storage.sync.get("id4", function(data){
                  var saeki_wid = String(Number(data.id4)+120);
                  redirector.setWtId('')
                  WixU.main.function.attach(saeki_wid, WixU.option.get('Main', 'minLength'));
                });
              }
